      ...
      <table class="table text-center table-striped table-bordered" style="width:100%">
        ...
        <tbody>
          <tr v-for="(sale, index) in filteredSales" :key="sale.saleId">
            ...
            <td>(@1) {{ sale.price.toFixed(2) }}</td>
            <td>{{ sale.amount_sold.toFixed(2) }}</td>
            <td>{{ getCategoryName(sale.categoryId) }}</td> 
            <td>{{ sale.customerName }}</td>
            <td><button class="btn btn-block" style="color:white;background-color:#beaa2b "> {{sale.saleStatus }}</button></td>
            <td><button class="btn btn-block" style="color:white;background-color:#224534">{{ sale.paymentMethod }}</button></td> <!-- Display payment method -->
          </tr>
        </tbody>
      </table>
